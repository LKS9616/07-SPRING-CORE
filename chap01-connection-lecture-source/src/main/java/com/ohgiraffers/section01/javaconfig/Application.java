    ...
    * */
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        // Enviornment : DB 적속에 관한 환경
        Environment enviornment  = new Environment("dev", //환경 정보 이름
                new JdbcTransactionFactory(), //트랜잭션 매니저 결정(JDBC or MANAGED)
                new PooledDataSource(DRIVER, URL, USER, PASSWORD)); // ConnectionPool 사용 유무(Pooled or UnPooled)

        // 순서 2. Configuration 객체에 환경정보 담기
        Configuration configuration = new Configuration(enviornment); // mybatis 설정객체
        ...
        * SqlSessionFactoryBuilder : SqlSessionFactory 인터페이스 타입의 하위 구현 객체를 새엉하기 위한 빌드 역할
        ...
        * - false : Connection 인터페이스 타입의 객체로 DML 수행 후 Auto commit에 대한 옵션을 false로 지정
        * - true : Connection 인터페이스 타입 객체로 DML 수행 후 Auto commit에 대한 옵션을 true로 지정
        * */

        SqlSession sqlSession = sqlSessionFactory.openSession(false);

        // getMapper() : configuration에 등록된 매퍼를 동일 타입에 대해 반환해준다.
